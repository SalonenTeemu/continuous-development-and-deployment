...
PORT = 5000
SERVICE2_URL = "http://service2_v1_1:5001/status"
...
def create_status_record():
    ...
    text_string = f"Service1@{timestamp}: uptime {uptime_minutes} minutes, free disk in root: {free_mb} MBytes"
...
def get_log():
    try:
        ...
        lines = []
        for entry in logs:
            uptime_minutes = round(entry.get("uptime_seconds", 0) / 60)
            free_mb = entry.get("free_mb", 0)
            timestamp = entry.get("timestamp", "")
            service = entry.get("service", "Unknown")
            lines.append(f"{service}@{timestamp}: uptime {uptime_minutes} minutes, free disk in root: {free_mb} MBytes")
